...
$(document).ready(function(){
    ...
    $("form#myForm").submit(function(event){
        ...
        $(".new-design").each(function(){
            ...
            var thisServing=[];
            var thisGreens=[];            
            $(this).find('input[name="snack"]:checked').each(function(){
                thisSnack.push(this.value);
            });
            $(this).find('input[name="drink"]:checked').each(function(){
                thisDrink.push(this.value);
            });
            $(this).find('input[name="stew"]:checked').each(function(){
                thisStew.push(this.value);
            });
            $(this).find('input[name="serving"]:checked').each(function(){
                thisServing.push(this.value);
            });
            $(this).find('input[name="greens"]:checked').each(function(){
        ...
        var validateResult=validate(orderSnacks,orderDrinks,orderStew,orderServings,orderGreens); 
        if (validateResult===false){
            return false;
        }
        var orderObject=new MealOrder(orderSnacks,orderDrinks,orderStew,orderServings,orderGreens);
...
function validate(snacks,drinks,stew,servings,greens){
    if(snacks.length<1 && drinks.length<1 && stew.length<1 && servings.length<1 && greens.length<1){
        $(".checkoutHide").hide();
...
function ordersDisplay(orderObject){
    ...
    for(var orderNum=0; orderNum<orderObject.snacks.length; orderNum+=1){
        ...
        var orderPrice=priceDeterminer(orderSn,orderDr,orderSt,orderSv,orderGr);                
    ...
    $(".no").click(function(){
        ...
        $(".yourOrder").append('<div class="yourAddress">'+                                        
                                        '<div class="row">'+
                                            '<div class="col-12">'+
                                                '<span class="grand-total">Grand Total: Ksh. '+grandTotalPrice()+'</span>'+
                                            '</div>'+
                                        '</div>'+
                                    '</div>');
    });
    $(".yes").click(function(){        
        ...
        $(document).on("submit","#deliveryForm",function(e){
            ...
            $(".deliveryDiv").remove();
            var addressObject=new Address(inputtedFName,inputtedLName,inputtedRoom);
            $(".yourOrder").append('<div class="yourAddress">'+
                                        '<div class="row">'+
                                            '<div class="col-12">'+
                                                '<h5>Your orders will be delivered here after payment:</h5>'+
                                                '<p>Name: '+addressObject.fullName()+'</p>'+
                                                '<p>Room: '+addressObject.room+'</p>'+                                                
                                            '</div>'+
                                        '</div>'+
                                        '<div class="row">'+
                                            '<div class="col-12">'+
